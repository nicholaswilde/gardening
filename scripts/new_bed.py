...
def main():
    if len(sys.argv) < 2:
        print("Error: Provide a filename (e.g., python3 scripts/new_bed.py raised-bed-4)")
        sys.exit(1)
    ...
    template = env.get_template("bed.md.j2")
    content = template.render(filename=filename, current_date=current_date)
    
    # Define output path
    out_dir = os.path.join(project_root, "docs", "beds")
