---
tags: [active, tree]
location: backyard
planted: 2021-11-01
botanical_name: Afrocarpus gracilior
family: Podocarpaceae
genus: Afrocarpus
---

# :seedling: Fern Pine

=== "2026-06-10"

    ![fern-pine][1]{ width="400" loading=lazy }

!!! example ""

    **:material-leaf: Type:** Evergreen Tree

    **:material-tag-text-outline: Botanical Name:** *Afrocarpus gracilior*

    **:material-sitemap-outline: Family:** Podocarpaceae

    **:material-folder-outline: Genus:** Afrocarpus

    **:material-dna: Variety:** Common

    **:material-storefront-outline: Origin:** Nursery Start

    **:material-map-marker-outline: Location:** Backyard

    **:material-calendar-check-outline: Date Planted:** 2021-11-01 (Late Fall / Winter)

    **:material-list-status: Status:** Active Growth

    **:material-weather-sunny: Sunlight:** Full sun to partial shade.

    **:material-texture: Soil:** Rich, moist, well-draining soil.

## :clipboard: Cultivation Status

## :memo: Log & Observations

* **2021-11-01:** Initial logging. Planted in the Backyard.

## :pushpin: Notes

* **Trefle Database Info:**
    * **Scientific Name:** *Afrocarpus gracilior* (Podocarpaceae Family)

[1]: <../assets/images/fern-pine-2026-06-10.webp>
