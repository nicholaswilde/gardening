---
tags: [annual, crop, archived]
location: raised-bed-2
planted: 2025-10-15
removed: 2026-05-10
source: seed
botanical_name: Allium schoenoprasum
family: Amaryllidaceae
genus: Allium
---

# :herb: Chives Common

!!! example ""

    **:material-leaf: Type:** Perennial Herb

    **:material-tag-text-outline: Botanical Name:** *Allium schoenoprasum*

    **:material-sitemap-outline: Family:** Amaryllidaceae

    **:material-folder-outline: Genus:** Allium

    **:material-dna: Variety:** Common

    **:material-store-outline: Origin:** Living herb

## :clipboard: Cultivation Status

| Attribute | Details |
| :--- | :--- |
| **Final State** | Harvested / Cleared |
| **Date Planted** | 2025-10-15 |
| **Date Removed** | 2026-05-10 |
| **Location** | Raised Bed 3 |
| **Outcome** | High yield. |
| **Origin** | Living herb |

## :memo: Log & Observations

## :pushpin: Notes

* **Trefle Database Info:**
    * **Scientific Name:** *Allium schoenoprasum* (Amaryllidaceae Family)
    * **Light Level:** 9/10
    * **Preferred Soil pH:** 7.5 - 8.0
